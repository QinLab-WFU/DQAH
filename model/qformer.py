...
class HashLayer(nn.Module):
    def __init__(self, num_features,hash_bit):
        super(HashLayer, self).__init__()
        self.head = nn.Linear(num_features, 1024)
...
class QuadrangleAttention(nn.Module):
    ...
    def forward(self, x, h, w):
        ...
        if h > ws or w > ws:
            ...
            zero_vector = torch.zeros(b*self.num_heads//self.window_num, window_num_h, window_num_w).cuda(1)
            sampling_projc = torch.cat([
                sampling_projc.reshape(-1, window_num_h, window_num_w, 1, 2),
                torch.ones_like(zero_vector).cuda(1).reshape(-1, window_num_h, window_num_w, 1, 1)
                ], dim=-1)

            shear_matrix = torch.stack([
                torch.ones_like(zero_vector).cuda(1),
                sampling_shear[..., 0],
                sampling_shear[..., 1],
                torch.ones_like(zero_vector).cuda(1)], dim=-1).reshape(-1, window_num_h, window_num_w, 2, 2)
            scales_matrix = torch.stack([
                sampling_scales[..., 0],
                torch.zeros_like(zero_vector).cuda(1),
                torch.zeros_like(zero_vector).cuda(1),
                sampling_scales[..., 1],
            ...
            window_coords_pers = torch.cat([
                window_coords.flatten(-2, -1), torch.ones(1, window_num_h, window_num_w, 1, ws*ws).cuda(1)
            ], dim=-2)
...
class QFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, rpe='v1', coords_lambda=0, hash_bit= 0,**kwargs):
        super().__init__()
        ...
        self.hash_layer = HashLayer(self.num_features, hash_bit=hash_bit)
